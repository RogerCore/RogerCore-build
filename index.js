 ...
 * <ul>
 * <li> `browser:uncompressed` - build uncomprssed browser bundle (`rogercore-*.js`)
 * <li> `browser:compressed` - build compressed browser bundle (`rogercore-*.min.js`)
...
function startGulp(name, opts) {
  ...
  var fullname = name ? 'rogercore-' + name : 'rogercore';
  var files = ['lib/**/*.js'];
  var tests = ['test/**/*.js'];
  var alljs = files.concat(tests);

  var buildPath = './node_modules/rogercore-build/';
  ...
  if (browser) {
    ...
    if (name !== 'lib') {
      browserifyCommand = buildBinPath + 'browserify --require ./index.js:' + fullname + ' --external rogercore-lib -o ' + fullname + '.js';
    } else {
      browserifyCommand = buildBinPath + 'browserify --require ./index.js:rogercore-lib -o rogercore-lib.js';
    }
  ...
  var release = function(importance, cb) {
    ...
    return runsequence(
      ...
      // Run git push RogerCore $VERSION
      'release:push-tag',
      // Run npm publish
      'release:publish',
